...
from meta_agent_v2.prompts import EXTRACTION_PROMPT
from meta_agent_v2.state import Goal, Value, ActionSuggestion, MetaAgentState, UserProfile
...
def identify_conflicting_values(user_profile: UserProfile) -> tuple[str, str] | None:
    """Identify two values that might conflict based on common value tensions.
    
    Args:
        user_profile: User profile with values

    Returns:
        Tuple of two conflicting value names, or None
    """
    values = list(user_profile.values.keys())
...
def format_values_summary(user_profile: UserProfile) -> str:
...
def format_goals_summary(user_profile: UserProfile) -> str:
    ...
    lines = []
    for goal_id, goal in user_profile.goals.items():
